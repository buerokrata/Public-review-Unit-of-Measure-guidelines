...
Beyond the reference to well-established unit of measure standards, there are three novelties addressed in this guideline:
...
Dimensional analysis is a convenient tool to build a rigorous system of measurement and serve as a validation tool for calculations and comparisons. The main rules of dimensional analysis/calculus can be summarised as follows:
...
However in the case of a 'Debt to reserves ratio', i.e. a scenario in which we calculate ratios of quantities that share the same unit of measure (and hence are commensurable) we have two competing representation options (for the same figure[^1]). 
...
We still divide debt with reserves, but this time we recognise that the two quantities are commensurable, so the resulting quantity - the same ratio as above - can be interpreted as the quantity in the numerator expressed in multiples of the quantity in the denominator.
...
From a contextuality point of view the unitless number approach yields the least precise, most contextual unit of measure. Assessing the magnitude is entirely left to the context. One must analyse the quantity kinds involved, the calculation methodology to be able to assess limits of comparability and additivity. A change of unit of measure approach offers the possibility to be more precise, less ambiguous, and delimit better the usage scope of the numbers. Nonetheless several options present themselves in the formulation of the units of measure, and they come with more or less unit information left for the broader context. 
...
- Activate the dimensional analysis benefits (set bounds of comparability and additivity), this corresponds to low contextuality, or precise and unambiguous formulation
...
The aim is that both the _Measure_ and the _Unit of measure_ emerge as a subset of Dimensions and Attributes, in an unambiguous and complete way. That is, each component (Dimension or Attribute) is either part of the _Measure_ or part of the _Unit of measure_ and the association of the component is consistent throughout a cube. This exercise, though it seems trivial, in practice can cause surprising challenges.

## 3.1 Should units of measure be modelled as dimensions or as attributes?

To best answer this question, one should ask why one would choose a component to be a dimension or an attribute in a Data Structure Definition in general.
...
Currency codes are often part of the narrow unit of measure code-list, yet there are two well identified modelling use cases when the separate representation of currency (concept and code-list) is needed.

1. When currency is used as a denomination currency (and not as a valuation currency) it has no impact on the unit of measure
...
Unit multipliers are used for readability, and they also double as an implicit measurement precision indicator (i.e., an economic value presented in millions is unlikely to be precise to the cent). In the SDMX code-list for unit multipliers the codes are powers of 10 needed to construct the actual multiplier. When the unit multiplier is 1, it can be omitted.
...
For this reason, the case insensitive symbols are defined. Although the Unified Code for Units of Measure does not encourage use of case insensitive symbols where not necessary, in some circumstances the case insensitive representation may be the greatest common denominator. Thus, some systems that can handle case sensitivity may end up using case insensitive symbols to communicate with less powerful systems.

To avoid ambiguity, it is also important to use “Code item” annotations to add alternative representation of symbols. As an example, an annotation should be added to reference the official UCUM, the SI-reference point and many of the popular unit of measure ontologies. When referring to kilograms, the code is set to “KG” in the proposed Units of Measure code-list. Since the official UCUM representation is “kg”, the following annotation would decorate the “Code item”.
...
### 3.3.2 Types of units of measure, coding patterns and labelling
...
These are the units of measure that are usually formed as a result of combining currency and scientific/itemizable units of measure (by applying dimensional analysis), e.g., National currency per person, Euros per hour, US dollars per tonne, Days per week, Kilogrammes per person.
